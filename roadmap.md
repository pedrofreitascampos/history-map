...
|---|---|---|---|
| **Full-surface audit + acceptance suite** | **2026-06-26** | **+73 (1883)** | **9 parallel auditors over every flow/route/view. 15 confirmed bugs fixed & live-verified: `cost` silently dropped by the server allowlist (whole cost feature dead) · bulk import bypassed lat/lng bounds · admin password reset didn't revoke sessions · trip delete never unlinked transits · markerHash omitted popup fields (stale popups) · Ctrl+K wiped unsaved edits · enrichment wrote into the wrong place's form · `setIf('loc-category')` targeted a nonexistent element · bulk edit cleared selection on partial failure · name/address unsanitised + uncapped · login timing enumeration + 500 on SSO accounts · malformed JSON → 500. New behavioural suites: acceptance-crud-roundtrip / -validation / -integrity-authz (IDOR matrix, cascade, share-token lifecycle). Two existing tests had encoded bugs as spec — inverted, not widened. Audit report: see memory.** |
