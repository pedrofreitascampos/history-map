...
Open items at the top so a fresh session can pick without scrolling. Shipped
detail (full audit table + chronological batch log) lives in the archive at
the bottom. Per-batch session log + commit detail in the memory roadmap at
`~/.claude/projects/C--Users-pedro-projects-software-history-map/memory/project_roadmap.md`.

Convention (companion `docs/architecture.md`):
- This file — Oikumene-specific concerns.
- Personal life → `~/.claude/personal/roadmap.md` (gitignored).
...
- **Direct share-from-Google-Maps → Oikumene** (2026-06-03 user ask). Two surfaces, shared server endpoint + URL parser:
  - **Mobile PWA share-target** — `manifest.json` `share_target` member → Maps share sheet posts `title + text + url` to a new `POST /api/import/google-maps-link` endpoint. Parses single place / saved list / `maps.app.goo.gl` short-link (HEAD expand with the same SSRF guard + 10s timeout used by web-import) and either opens the Add modal pre-filled or runs the list importer. Blocked on Oikumene becoming installable (Service Worker — overlaps with Power feature #7).
  - **Desktop bookmarklet** — one-line `javascript:` bookmarklet on `google.com/maps/...` reads `window.location.href` + page title → opens `https://history-map.onrender.com/#add?url=<encoded>` → `#add?url=` hash handler routes into Add modal or list importer. No extension needed; works on Chrome/Firefox/Safari/Edge today. Install card under Account modal: "Drag this button to your bookmarks bar."
  - Implementation tree: (1) URL parser grammar, (2) shortlink expander with SSRF guard, (3) `#add?url=` hash handler in `switchView` + `init()`, (4) PWA manifest + Service Worker, (5) bookmarklet install card.
- **Bootstrap preset collections** (2026-06-03 user ask) — library of famous ready-made collections: **UNESCO World Heritage Sites** (~1200, GeoJSON from whc.unesco.org), **National Parks** (per country: NPS US, ICNF PT, …), **Airports** (OurAirports CSV, ~50k IATA-coded), **Stadiums** (Wikidata SPARQL `P31 = stadium/arena`), **Wonders of the World** (3 curated lists × 7), **Blue Flag beaches** (annual per country). Michelin Guide is ToS-grey — defer. Sketch: `/api/collections/presets` metadata catalog + per-preset adapter in `server/preset-collections/` analogous to `import-adapters/`. Versioned (re-fetch yearly). **Open design question:** do preset locations live in user's `state.locations` (clutters wishlist/been views) or as a separate overlay layer toggled per-collection? Probably overlay-first to keep the personal list clean.
- **Bootstrap from sources without an export API** — Playwright/headless-browser scraper pattern. Adapter slot in `WEBSITE_IMPORT_ADAPTERS` open for per-target use. Targets case-by-case (beliapp.co is blocked, see below).
- **Google Photos Path 2** (photo-org bridge) — Path 3 (manual EXIF drop) ✅ shipped 2026-05-31 in `c417584`. Path 2 requires repaired photo-org (DB columns + NAS path pivot since Google scope removal). Queued behind photo-org repair.
- **Google Data Portability API OAuth** — blocked on Google's Restricted-scope verification for personal apps. Monitor for relaxation.
- **Strict `style-src` CSP** — Leaflet injects nonceless inline styles; per CSP-3, mixing `'unsafe-inline'` + nonce makes browsers ignore `'unsafe-inline'`. Accepted defense-in-depth gap until strict-dynamic-for-CSS or Leaflet replacement.

## 🟡 P2 — Polish backlog

Worth tracking but not blocking. Pick opportunistically when adjacent work makes them cheap.

**Security INFO/LOW (5)** — `revokedJtis` in-memory only (persist to nedb if tightening); dev fallback JWT secret hardcoded (warn in non-prod, randomize per startup); KMZ zip-slip belt-and-suspenders guard (`!n.includes('..')`); `express ^4.18.2` transitive `path-to-regexp` HIGH + `qs` MODERATE — `npm update express` should clear; missing `.env.example`.

**Code-quality (3)** — `render.yaml` free tier wipes data on deploy (upgrade Starter + persistent disk); no ESLint with `no-undef` to catch implicit globals; `logout` swallows errors silently. (`loadFromServer` blocks `initMap` ✅ closed 2026-06-04.)

**UX MEDIUM/LOW**
- Theme switch needs preview — single 🌙 button cycles themes blind; no `prefers-color-scheme` integration.
- Category colors not part of theme swap — `--cat-restaurant: #ff6b6b` stays constant; in Volcano theme red accent + red restaurant marker collide.
...
- Regions view drill-down — ~~clicking does nothing~~ ✅ partly closed 2026-06-03 (Country/Region/City popups list visited places). Still open: filter map view by region click, drill-down zoom.
...
- `#map-search-results` persists with stale text after sidebar input loses focus (partial overlap with the 2026-06-11 provider-change clear — confirm before picking).
- Ad-hoc spacing (8/10/12/14/16/20/24 px) — no 4px-base grid declared.

**Live functionality LOW** — drag-drop no undo, wishlist sort on keyboard nav, autocomplete persistence after blur, category emoji collision (overlap with UX above).
...
2. **🗺️ Stadia tiles + theme-aware tile swap** — instant premium feel; OSS-friendly. (Also listed under UX P1.)
...
7. **📡 Offline / PWA mode** — Service Worker + tile cache. Travel app unusable on planes / remote areas today. (Unblocks the Google-Maps share-target above.)
8. **🔍 Smart import deduplication** (Notion "this page exists") — fuzzy-match incoming names vs existing locations (Levenshtein < 0.2 + within 500 m) before commit.
9. **⏱️ Time-of-day heatmap** (Strava activity heatmap) — "you visit restaurants mostly Fri-Sat 8-10 pm". Uses existing `visits[].date`.
...
16. **🕸️ Graph view on a renamed "Atlas" tab** (2026-06-03 user ask). Current Regions tab is misnamed — it already does Country / Region / City. Rename to **Atlas** / **Geography** / **Spatial**. Add a 4th segmented option: **🕸️ Graph** — node-link diagram, one node per visited place (positioned by lat/lng over dark basemap), directed edges between consecutive chronological visits. Node size = visit count (`sqrt(pop)` like City view); edge weight = visit count per pair; edge color by transit mode when a matching `transits` record exists. Hover edge → tooltip with date+mode. Variants: **Time window** (all-time / per-year / per-trip) + **Density** (all edges vs top-N busiest). Pure client-side over existing indexes — no new endpoints.
...
- **beliapp.co import** — architecturally blocked. `https://beliapp.co/app/<username>` is install-marketing only (App Store redirect; no web profile / JSON / SPA bootstrap). Gmail search across 9 angles found no exports. 2026-05-31 user-confirmed skip. If revisited: in-app export feature (status unknown) or 2-3 day Playwright+credentials scraper (ToS-grey + brittle + Chromium dep).
...
> The audit's original **🟠 P1 / 🟡 P2 / ✨ Power features / Sequenced ship plan** sections have been promoted to the top of this file — see [🟠 Open](#-open---pick-from-here), [🟡 P2](#-p2--polish-backlog), [✨ Power features](#-power-features), [Sequenced ship plan](#sequenced-ship-plan). Per-row closed/open detail and original auditor wording live in the memory roadmap at `~/.claude/projects/C--Users-pedro-projects-software-history-map/memory/project_roadmap.md`.
