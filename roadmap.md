...
- ~~"On This Day" → one-tap Replay~~ ✅ · ~~Year-in-Review shareable card~~ ✅ · ~~collection completion rings~~ ✅ · isochrone wishlist scoring (Valhalla already in CSP) · ~~`?` keyboard-shortcut overlay + ⌘K quick-add~~ ✅ · ~~LLM trip-journal from visits~~ ✅ · ~~per-category marker style~~ ✅.

| Batch | Date | Jest | Highlights |
|---|---|---|---|
| **Open-findings batch (High severity)** | **2026-08-30** | **+13 (1896)** | **Five High findings from the audit's Open register, now fixed & live-verified: Restore button pointed at the admin-only endpoint for every user (`restoreBackup` now hits `/api/my-backup/:filename`, checks `res.ok`, sends credentials) · bulk enrich/Google-sync paths mutated category/address in place without bumping `stateIndex.generation` (Stats/Regions stayed stale — fixed in `bulkEnrichPhoton`, `enrichInBackground`, `syncImportedLocations`, `bulkGoogleSync`) · bulk edit/delete/attach/collection ops fanned out one request per item unthrottled, able to trip the 200/min global limiter (`chunkedAllSettled` caps fan-out at 50 + 400ms gap; `api()` now surfaces a clear message on 429 instead of a generic error; `selectAllBulk` warns past 200) · neighborhood clustering ran an unbounded O(n²) pass over ALL locations on every zoom event (now bounded to the viewport, memoised on generation+zoom+bounds, respects active filters via `getFilteredLocations`) · session expiry silently reappeared the login screen (now toasts first). New `tests/audit-open-findings-batch.test.js`; 4 existing `Promise.allSettled` source-pins updated to assert `chunkedAllSettled` (still real invariant, not widened).** |
| **Full-surface audit + acceptance suite + E2E** | **2026-06-26** | **+88 (1883 jest + 22 e2e)** | **9 parallel auditors over every flow/route/view. 16 confirmed bugs fixed & live-verified: `cost` silently dropped by the server allowlist (whole cost feature dead) · bulk import bypassed lat/lng bounds · admin password reset didn't revoke sessions · trip delete never unlinked transits · markerHash omitted popup fields (stale popups) · Ctrl+K wiped unsaved edits · Add-modal Cancel silently discarded typed data (no dirty-check snapshot) · enrichment wrote into the wrong place's form · `setIf('loc-category')` targeted a nonexistent element · bulk edit cleared selection on partial failure · name/address unsanitised + uncapped · login timing enumeration + 500 on SSO accounts · malformed JSON → 500. New: acceptance-crud-roundtrip / -validation / -integrity-authz (IDOR matrix, cascade, share-token lifecycle) + 7 new Playwright specs (add-place, edit-freshness, Ctrl+K guard, trip lifecycle, bulk edit, empty states, mobile viewport). Two existing tests had encoded bugs as spec — inverted, not widened. Audit report: see memory.** |
