...
**Status as of 2026-06-11:** **10 of 10 original P0s + 13 of S2 P1s closed.** Audit P0 close-out complete. 2026-06-04 morning hardening (+16 jest, 986+3skip): narrate+discover rate limits, CSP photon, render.yaml ANTHROPIC_API_KEY, SDK exact-pin, notes sanitisation. 2026-06-04 afternoon perf cold-load (+14 jest, 1000+3skip): Cache-Control+ETag on /api/locations, initMap-before-await, defer Chart.js/TopoJSON/JSZip/exifr. 2026-06-11 live-fixes batch (+21 jest, **1056+3skip green**, 8/8 e2e): Photon-provider stale-results clear ✅, Narrate disabled-state UX ✅, password inputs in `<form>` with autocomplete ✅, pagination `data-click` migration ✅, weather overlay persist pinned (verified-not-a-bug). Next: surgical rebuildIndexes, DNS-rebinding SSRF, quick-add Photon shortcut.
...
- ~~**`more.onclick` direct assignment** in 5 callsites — migrate to `data-click` dispatcher; inconsistent with the project's CSP-compatible pattern.~~ ✅ **Partly closed 2026-06-11** — the 2 surface UI pagination buttons (`chrono-load-more`, `wishlist-load-more`) migrated to `data-click` via `setAttribute`. The other "onclick" hits are confirm/prompt-modal close handlers wired against freshly-created elements in `showConfirm`/`showPrompt` helpers — those are internal helper bookkeeping, not surface UI, and the `element.onclick = fn` JS-property assignment doesn't fall under the CSP `script-src-attr 'none'` block (only inline HTML `onclick="…"` does). Leave as-is.

**Perf round 2** — **3 of 8 closed in cold-load batch 2026-06-04 (+14 jest, suite at 1000+3skip green, 8/8 e2e green).**
- ~~**`GET /api/locations` no ETag / no `?since=`** — 5k locations = ~2MB JSON re-shipped on every refresh.~~ ✅ **Closed 2026-06-04** — `Cache-Control: no-cache` set; Express's automatic weak ETag (body hash) handles If-None-Match → 304. Browser disk cache serves the cached body on revalidation, saving ~140 KB gzip per refresh once warm. `?updatedSince=` deferred to a later round — the no-cache+ETag path is the 80% win and ships in 3 lines. 5 jest pins (Cache-Control, ETag present, 304 round-trip, insert invalidates etag, stale If-None-Match falls through).
- ~~**`initMap()` blocked by `loadFromServer()`** — swap order so tiles begin fetching immediately.~~ ✅ **Closed 2026-06-04** — `if (!map) initMap()` now runs BEFORE `await loadFromServer()` in startApp(); explicit `renderMarkers()` + `invalidateSize` run after the await so loaded data lands. Leaflet tile requests now race the /api/locations fetch — on a 200 ms RTT cold load that's ~200 ms shaved off first map paint. 2 jest pins (call-order assertion + post-await renderMarkers pin).
- ~~**8 CDN script tags render-blocking** (Leaflet, MarkerCluster ×2, Heat, Chart.js, TopoJSON, JSZip, exifr). Defer Chart.js / TopoJSON / JSZip / exifr (~300 KB blocking).~~ ✅ **Closed 2026-06-04** — `defer` added to all 4 optional CDN scripts (Chart.js / TopoJSON / JSZip / exifr); Leaflet + MarkerCluster + Heat stay blocking since initMap() needs them synchronously at startup. All 4 deferred libraries referenced only inside view-switch / event handlers (Stats view, Regions view, KMZ import, photo EXIF drop) — never at top-level — so defer is safe. 7 jest pins (4 defer-present + 3 still-blocking).
...
**Live functionality fixes** — **4 of 5 closed in batch 2026-06-11 (+21 jest, suite 1056+3skip green, 8/8 e2e). Weather overlay persist verified not a bug + pinned via regression.**
- ~~**Weather overlay state never persisted to localStorage on toggle ON** (only persists OFF). Confirmed: `activeOverlays: "[]"` even after toggle on.~~ ✅ **Closed 2026-06-11 (verified-not-a-bug).** Code inspection shows both branches in `toggleOverlay` mutate `state.activeOverlays` then call `_persistActiveOverlays()` (lines 4280 + 4295). Likely original report was from a stale build before the RainViewer registry ship (`20e00d7`). Added 3 jest pins guarding both attach + detach persist call sites plus the array-serialization contract so a future refactor can't silently break either path.
- ~~**Photon "not configured" leftover error** — `#search-note` still shows "Google Places not configured" even after user selects Photon provider. Update via `_refreshGoogleChromeVisibility` extension.~~ ✅ **Closed 2026-06-11** — `onSearchProviderChange` now wipes `#map-search-results` innerHTML + hides the container. (The "Google Places not configured" message actually renders into `#map-search-results`, not the roadmap-referenced `#search-note`; fix lands in the same path.)
...
- ~~**Trip narrate button** — no key warning until submit; show inline warning if `user.anthropicKey` unset and no `ANTHROPIC_API_KEY` env.~~ ✅ **Closed 2026-06-11** — both Narrate buttons (Trip Manager footer + Trips empty state CTA) carry `.narrate-btn`; new `_refreshNarrateButtonState()` queries `/trips/narrate-status` (session-cached, invalidated on save/remove), toggles `.narrate-btn-disabled` (opacity 0.55) + appends " — needs API key" to the label + retitles to point at Account settings. Fires on view-switch into Trips + after key save/remove. Existing `openNarrateModal` toast+redirect path stays as the fallback.
- ~~**Password inputs not inside `<form>`** — browser warns + blocks password-manager autofill on Places + Anthropic key fields.~~ ✅ **Closed 2026-06-11** — login wrapped in `<form id="auth-form" data-submit="doAuth" autocomplete="on">` with `autocomplete="username"` on the username input + `current-password` on the password. Places + Anthropic key inputs (both render paths) wrapped in `<form data-submit="…">` with `autocomplete="off"` (API keys, not passwords); Save buttons are `type="submit"`, Remove buttons keep `type="button"`. Submit dispatcher in `_delegate` always `preventDefault()` so no full-page POST. `enterToFocus` now `preventDefault()` too so Enter on username focuses password without double-firing the form submit.
...
| **S2** | Hardening + perf round 2 | ~~Per-endpoint rate limits, CSP `photon.komoot.io`, SDK pin, render.yaml, notes sanitisation, ETag on /api/locations, initMap-first, lazy non-map CDNs~~ ✅ (8 of S2 shipped 2026-06-04 across two batches). Remaining: surgical rebuildIndexes, getFilteredLocations memo, markerHash alloc, buildTagFilters DOM toggle, RainViewer persist fix, Photon-provider error label, DNS-rebinding SSRF, more.onclick migration | 1-2 days |
