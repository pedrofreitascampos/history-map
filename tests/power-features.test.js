...
// ──────────────────────────────────────────────────────────────────────────────
// 6. Plan-a-Day — static markup
// ──────────────────────────────────────────────────────────────────────────────
describe('Plan-a-Day — static markup', () => {
  test('_wishSelIds variable declared', () => {
    expect(indexHtml).toContain('_wishSelIds');
  });

  test('wish-sel-btn toggle button rendered in wishlist items', () => {
    expect(indexHtml).toMatch(/class="wish-sel-btn[\s\S]{0,100}data-click="toggleWishSel"/);
  });

  test('#plan-day-bar exists with display:none initially', () => {
    expect(indexHtml).toContain('id="plan-day-bar"');
    expect(indexHtml).toMatch(/id="plan-day-bar"[^>]*display:none/);
  });

  test('#plan-day-btn uses data-click="planADay"', () => {
    expect(indexHtml).toMatch(/id="plan-day-btn"[\s\S]{0,100}data-click="planADay"/);
  });

  test('#plan-day-modal exists with display:none', () => {
    expect(indexHtml).toContain('id="plan-day-modal"');
    expect(indexHtml).toMatch(/id="plan-day-modal"[^>]*display:none/);
  });

  test('#plan-day-name input inside modal', () => {
    const modalStart = indexHtml.indexOf('id="plan-day-modal"');
    const modalEnd = indexHtml.indexOf('</div>', modalStart + 600);
    const modalHtml = indexHtml.substring(modalStart, modalEnd + 6);
    expect(modalHtml).toContain('id="plan-day-name"');
  });

  test('Create Trip button uses data-click="confirmPlanADay"', () => {
    expect(indexHtml).toMatch(/data-click="confirmPlanADay"/);
  });

  test('Cancel button uses data-click="closePlanDayModal"', () => {
    expect(indexHtml).toMatch(/data-click="closePlanDayModal"/);
  });

  test('all plan-a-day functions defined', () => {
    ['toggleWishSel', 'clearWishSel', '_updatePlanDayBtn', 'planADay',
      'closePlanDayModal', '_nearestNeighborOrder', 'confirmPlanADay'].forEach(fn => {
      expect(indexHtml).toContain(`function ${fn}`);
    });
  });

  test('_updatePlanDayBtn called at end of renderWishlist', () => {
    const fn = extractFunction('renderWishlist');
    expect(fn).toContain('_updatePlanDayBtn');
  });

  test('Escape closes plan-day-modal', () => {
    const escBlock = indexHtml.match(/key !== 'Escape'[\s\S]{0,2000}const modals = \[/);
    expect(escBlock).not.toBeNull();
    expect(escBlock[0]).toContain('plan-day-modal');
    expect(escBlock[0]).toContain('closePlanDayModal');
  });

  test('.wish-sel-btn CSS rule exists', () => {
    expect(indexHtml).toMatch(/\.wish-sel-btn\s*\{/);
  });

  test('.wish-sel-btn.sel CSS rule exists', () => {
    expect(indexHtml).toMatch(/\.wish-sel-btn\.sel\s*\{/);
  });
});

// ──────────────────────────────────────────────────────────────────────────────
// 7. Plan-a-Day — nearest-neighbor logic (vm sandbox)
// ──────────────────────────────────────────────────────────────────────────────
describe('Plan-a-Day — routing logic', () => {
  function runNN(matrix) {
    const code = [
      extractFunction('_nearestNeighborOrder'),
      `__result = _nearestNeighborOrder(${JSON.stringify(matrix)});`,
    ].join('\n');
    const ctx = vm.createContext({ Math, Number, Array, Object, Set, Infinity, __result: null });
    vm.runInContext(code, ctx);
    return ctx.__result;
  }

  test('single location returns [0]', () => {
    expect(runNN([[0]])).toEqual([0]);
  });

  test('two locations returns order of length 2', () => {
    const order = runNN([[0, 5], [5, 0]]);
    expect(order).toHaveLength(2);
    expect(order).toContain(0);
    expect(order).toContain(1);
  });

  test('nearest-neighbor picks shortest first leg', () => {
    // From 0: dist to 1 = 10, dist to 2 = 3 → should visit 2 before 1
    const matrix = [
      [0, 10, 3],
      [10, 0, 4],
      [3, 4, 0],
    ];
    const order = runNN(matrix);
    // start: best outgoing sum → 0 (3+10=13) vs 1 (10+4=14) vs 2 (3+4=7) → starts at 2
    // or could start at 0 depending on tie-breaking; what matters is order covers all 3
    expect(order).toHaveLength(3);
    expect(new Set(order).size).toBe(3); // all unique
  });

  test('all indices covered for 4-node matrix', () => {
    const matrix = [
      [0, 2, 9, 10],
      [1, 0, 6, 4],
      [15, 7, 0, 8],
  ...
  test('nearest-neighbor prefers closer node (greedy)', () => {
    // Simple chain: 0→1 is 1, 0→2 is 100 — should visit 1 before 2
    const matrix = [
      [0, 1, 100],
      [1, 0, 1],
      [100, 1, 0],
    ...
    const order = runNN(matrix);
    // Whatever the start, consecutive visits should prefer the nearest unvisited
    const idx1 = order.indexOf(1);
    const idx2 = order.indexOf(2);
    // 1 and 2 are adjacent in the chain; just verify all present
    expect(order).toHaveLength(3);
